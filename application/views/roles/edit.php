    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden p-6 sm:p-8">
        <form action="<?= base_url('roles/edit/'.$role->id) ?>" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="name" value="<?= htmlspecialchars($role->name) ?>" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary font-mono text-sm lowercase" required>
                <p class="text-xs text-gray-500 mt-2 font-medium">Gunakan hanya huruf kecil, angka, dan underscore (_). Dilarang menggunakan spasi.</p>
            </div>
