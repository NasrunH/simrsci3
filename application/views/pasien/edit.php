<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden p-6 sm:p-8">
        ...
        <!-- Info Rekam Medis Statis -->
        <div class="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-200 flex flex-col md:flex-row md:items-center justify-between">
            <div>
                <p class="text-xs text-gray-500 font-semibold uppercase tracking-wider">Nomor Rekam Medis</p>
                <p class="text-xl font-bold text-primary"><?= htmlspecialchars($pasien->no_rekam_medis) ?></p>
            </div>
        ...
        <form action="<?= base_url('pasien/edit/'.$pasien->id_pasien) ?>" method="POST">
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="nama_lengkap" value="<?= htmlspecialchars($pasien->nama_lengkap) ?>" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary" required>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Tanggal Lahir <span class="text-red-500">*</span></label>
                        <input type="date" name="tanggal_lahir" value="<?= $pasien->tanggal_lahir ?>" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary" required>
                    </div>
                ...
                <div>
                    ...
                    <textarea name="alamat" rows="4" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary"><?= htmlspecialchars($pasien->alamat) ?></textarea>
                </div>
