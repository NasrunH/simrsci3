<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800">Edit Layanan Medis</h1>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sm:p-8">
        <form action="<?= base_url('layanan/edit/'.$layanan->id_layanan) ?>" method="POST">
            <div class="mb-5">
                ...
                <input type="text" name="nama_layanan" value="<?= htmlspecialchars($layanan->nama_layanan) ?>" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary text-sm" required>
            </div>

            <div class="mb-5">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Tarif Jasa Medis (Rp) <span class="text-red-500">*</span></label>
                <input type="number" name="tarif" value="<?= $layanan->tarif ?>" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary text-sm font-bold" required>
            </div>

            <div class="mb-6">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Deskripsi Layanan</label>
                <textarea name="deskripsi" rows="3" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary text-sm"><?= htmlspecialchars($layanan->deskripsi) ?></textarea>
            </div>
