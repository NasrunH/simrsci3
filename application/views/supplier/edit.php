<div class="max-w-xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sm:p-8">
        ...
        <form action="<?= base_url('supplier/edit/'.$s->id_supplier) ?>" method="POST">
            
            <div class="mb-5">
                <label class="block text-sm font-semibold text-gray-700 mb-2">ID Register Supplier</label>
                <input type="text" value="SPL-<?= str_pad($s->id_supplier, 4, '0', STR_PAD_LEFT) ?>" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm font-bold text-gray-500 font-mono uppercase" readonly tabindex="-1">
            </div>

            <div class="mb-5">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Perusahaan / Supplier <span class="text-red-500">*</span></label>
                <input type="text" name="nama_supplier" value="<?= set_value('nama_supplier', $s->nama_supplier) ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-primary focus:border-primary" required>
            </div>

            <div class="mb-5">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Nomor Telepon Kantor / Sales</label>
                <input type="text" name="no_telp" value="<?= set_value('no_telp', $s->no_telp) ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-primary focus:border-primary">
            </div>

            <div class="mb-6">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Alamat Lengkap Perusahaan</label>
                <textarea name="alamat" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-primary focus:border-primary"><?= set_value('alamat', $s->alamat) ?></textarea>
            </div>
